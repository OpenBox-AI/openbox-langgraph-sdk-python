...
1. Research directly with the `web_search` tool (run several specific queries if the topic is broad)
2. Gather at least 3 credible sources
3. Save your findings to `research/<slug>.md` with `write_file`, then re-read them with `read_file` while drafting
4. Identify the key points readers need to understand
5. Find concrete examples or case studies to illustrate concepts
