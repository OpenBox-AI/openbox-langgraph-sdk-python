...
**Before writing any blog post, you MUST research the topic yourself:**

1. Call `web_search` directly with specific queries (run several if the topic is broad)
2. Save the key findings with `write_file` to `research/<slug>.md`

Example:

```
web_search(query="current state of AI agents in 2025 adoption trends")
write_file(file_path="research/ai-agents-2025.md", content="<summarized findings with sources>")
```

3. After saving, read the findings file with `read_file` before writing
...
1. Write the post to `blogs/<slug>/post.md` with `write_file`
