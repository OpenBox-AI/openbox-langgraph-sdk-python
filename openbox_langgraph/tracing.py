...
The spans carry function metadata (name, args, result) as OTel attributes.

Hook governance is NOT applied here: it is owned entirely by the shared
``openbox_core`` base instrumentation (installed via ``create_core_runtime``).
``@traced`` is a plain OTel tracing helper — it never builds hook payloads or
evaluates governance verdicts inline.
