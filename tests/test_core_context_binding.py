...
async def test_tool_start_registers_context_in_base_registry(make_handler) -> None:
    """After ToolStarted processing, the base TraceContextRegistry resolves the
...
async def test_tool_completion_updates_then_clears_base_store(make_handler) -> None:
    """ToolCompleted: the base registry updates the trace to the `-c` completed
    identity, THEN unregisters it (span lifetime ends here) — see
    langgraph_handler.py's on_tool_end comment for why update-then-unregister
